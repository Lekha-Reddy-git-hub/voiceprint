---
name: voiceprint
description: Learn a person's writing voice from their own samples and render their raw thoughts as finished prose in that voice — with an evidence-based audit report. Use when the user wants writing that sounds like them, wants their notes/thought-dumps turned into posts/emails/essays in their own style, or asks "does this sound like me / like AI?"
---
...
third person is out of scope — decline it.

## Organ 1 — PROFILE (run once, update often)

1. Ask for 5–10 samples of their real, finished writing (same genre as the target:
   LinkedIn voice ≠ email voice ≠ essay voice). ALSO ask for 2–3 raw thought-dumps
   or before/after edit pairs if they have them — idea-shape matters as much as
   sentence-shape.
...
6. Write the profile as IMPERATIVES, not description ("Open mid-thought. Never use
   semicolons. End ~1 in 3 posts on an unresolved question."), and embed 2–3 short
   excerpts from the real samples as exemplars.
...
Pass A — content: organize their ideas. Add nothing they didn't think. Where the
  draft needs a lived anecdote, insert `[YOUR EXAMPLE: …prompt…]` — never fabricate
  their experiences.
...
Pass C — de-slop: strip machine tells (pair with a slop-removal skill if installed;
  otherwise apply the built-in minimal list: no "delve/robust/seamless/tapestry",
  no "isn't just X", no throat-clearing openers, no restate-everything endings).

## Organ 3 — AUDIT (attach to every render; receipts, never verdicts)

Re-run `style_vector.py` on the draft. Report:
- Each metric vs the user's OWN baseline (never vs a universal ideal — universal
  rubrics create monoculture; see RESEARCH.md).
- Regularity check: which of the user's habits is the draft over-repeating?
  ...
  signature move that exceeds ~1.5× the user's baseline frequency.
...
- They deleted your em-dashes → lower the em-dash cap.
